...
export function Login() {
  ...
  const handleGoogleSignIn = async () => {
    ...
    try {
      ...
      // Error lain yang perlu ditampilkan
      ...
      setError(errorMessages[err.code] || 'Gagal masuk dengan Google. Coba lagi.');
    } finally {
      setLoading(false);
    }
  };
    ...
    <div className="w-full min-h-screen bg-[#f8f7f4] text-[#1d2421] flex items-center justify-center overflow-y-auto font-sans relative py-8">
      ...
      <div className="w-full max-w-md p-8 bg-white border border-[#e4e1da] rounded-3xl shadow-xl shadow-[#0f6e56]/5 relative z-10 m-4">
        <div className="text-center mb-10">
          ...
          <p className="text-[#777670] mt-4">Masuk untuk mengelola finansialmu.</p>
        </div>
        ...
        <div className="relative"><div className="absolute inset-0 flex items-center"><div className="w-full border-t border-[#e4e1da]" /></div><div className="relative flex justify-center text-sm"><span className="px-3 bg-white text-[#777670]">atau masuk dengan</span></div></div>
        <button type="button" onClick={handleGoogleSignIn} disabled={loading} aria-label="Masuk dengan Google" title="Masuk dengan Google" className="w-12 h-12 mx-auto flex items-center justify-center border border-[#dedbd4] rounded-xl bg-white hover:bg-[#f7f6f2] hover:border-[#acd3c7] focus:outline-none focus:ring-2 focus:ring-[#0f6e56]/20 disabled:opacity-50 transition-all shadow-sm">
        ...
        <div className="text-center mt-6">
          <Link to="/register" className="text-sm text-[#777670] hover:text-[#0f6e56] transition-colors">Belum punya akun? <span className="text-[#0f6e56] font-bold">Daftar sekarang</span></Link>
        </div>
      </div>
    </div>
  );
